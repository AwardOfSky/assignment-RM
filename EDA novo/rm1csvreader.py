...
#init vars
clean_dataset = True
clean_dataset_nocutoff = True
show_avg_surface = True
show_std_surface = True
eda = True
debug = False
save_figure = True
show_plts = True # nao mexer nisto
        ...
        with open(filename, 'r', newline='') as ff:
            ...
            for row in reader:
                if lc >= line_start:
                    ...
                    if code_type == 0 or (code_type == 1 and str(row[2]) == "code1") or (code_type == 2 and str(row[2]) == "code2"):
...
if eda:
    ...
    ori = 'vertical'
    if n_hist_plot:
        df["N"].plot(kind="hist", orientation=ori, cumulative=False)
        plt_manage(eda_plots_dir + "n_hist.png")
    if time_hist_plot:
        df["Time"].plot(kind="hist", orientation=ori, cumulative=False)
        plt_manage(eda_plots_dir + "time_hist.png")

    if box_plt:
        color = dict(boxes='orangered', whiskers='orangered', medians = 'blue', caps = 'black')
        #df[["N", "Time"]].plot(kind="box", subplots = True, layout = (1, 2), sharex = False, sharey = False, figsize = (16, 8), color = color, sym='r+')
        df["N"].plot(kind="box", sharex = False, sharey = False, figsize = (8, 8), color = color, sym='r+')
        plt_manage(eda_plots_dir + "boxplot_n.png")
        df["Time"].plot(kind="box", sharex = False, sharey = False, figsize = (8, 8), color = color, sym='r+')
        plt_manage(eda_plots_dir + "boxplot_time.png")
